...
$relevanssi_variables['plugin_version'] = "1.14.9";
...
function relevanssi_pinning($hits) {
	...
	if (is_array($term_list)) {
		...
		$positive_ids = $wpdb->get_col($q);

		$q = "SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '_relevanssi_unpin' AND meta_value IN ($term_list)";
		$negative_ids = $wpdb->get_col($q);

		$pinned_posts = array();
		$other_posts = array();
		foreach ($hits[0] as $hit) {
			if (is_array($positive_ids) && count($positive_ids) > 0 && in_array($hit->ID, $positive_ids)) {
				$pinned_posts[] = $hit;
			}
			else {
				if (is_array($negative_ids) && count($negative_ids) > 0) {
					if (!in_array($hit->ID, $negative_ids)) {
						$other_posts[] = $hit;
					}
				}
				else {
